...
describe("loadTerminalAddons", () => {
  ...
  it("opens xterm links through desktop host bridge", async () => {
    ...
    mocks.webLinksOpenHandler?.({ preventDefault } as unknown as MouseEvent, "https://vestin.io/docs");

    expect(preventDefault).toHaveBeenCalledTimes(1);
    await vi.waitFor(() => {
      expect(mocks.openExternalUrl).toHaveBeenCalledWith("https://vestin.io/docs");
    });
